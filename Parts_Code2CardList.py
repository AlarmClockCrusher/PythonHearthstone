...
def checktheStatsofCards():
	raceAllCapDict = {"Elemental":"ELEMENTAL", "Mech": "MECHANICAL", "Demon": "DEMON", "Murloc": "MURLOC",
						"Dragon": "DRAGON", "Beast": "BEAST", "Pirate": "PIRATE", "Quilboar":"QUILBOAR", "Totem": "TOTEM",
	...
	for card in cardPool:
		...
		try:
			if cardType == "Minion":
				...
				if not (cardInfo["attack"] == card.attack == int(words[4]) and cardInfo["health"] == card.health == int(words[5])):
				...
				if "race" in cardInfo:
					...
					if cardInfo["race"] == "All":
						if card.race != "Elemental,Mech,Demon,Murloc,Dragon,Beast,Pirate,Quilboar,Totem":
							print(card, " race should be 'Elemental,Mech,Demon,Murloc,Dragon,Beast,Pirate,Quilboar,Totem'")
				...
				if not(cardInfo["attack"] == card.attack == int(words[4]) and cardInfo["durability"] == card.durability == int(words[5])):
					print(card, " has a wrong stat")
			else: #Spell
				if card.name != words[5]:
					print(card, " has a wrong name", cardInfo["name"], card.name, words[5])
				if "spellSchool" in cardInfo:
					if card.school != words[4] or schoolAllCapDict[card.school] != cardInfo["spellSchool"]:
						print(card, "has a wrong school", cardInfo["spellSchool"], card.school, words[4])
				elif card.school or words[4]:
					print(card, "shouldn't have school", card.school, words[4])
		except Exception as e:
...
def checkCardStats_Alltogether():
