...
class Btn_Card:
	...
	def reloadModels(self, loader, imgPath, pickable, onlyShowCardBack):
		...
		if category == "Dormant": isLegendary = "~Legendary" in card.minionInside.index if card.minionInside else True
		else: isLegendary = "~Legendary" in card.index
	...
	def effectChangeAni_Shared(self, name, para):
		...
		if not name or name == "Unplayable":
			para.append(Func(self.texCardAni_OnOff, "Unplayable", not self.isPlayed and card.inHand and card.effects["Unplayable"] > 0))
	...
	def setBoxColor(self, color):
		if self.box:
			try: #Unknown reason why this could sometimes
				if color == transparent: self.box.hide()
				else:
					self.box.show()
					self.box.setColor(color)
			except:
				print("=======Error when setting box color for card:", self.card, self.box)
				pass
	...
	def statChangeAni(self, num1=0, num2=0, action="set"):
		...
		color_Attack = white if card.attack <= card.attack_0 else green
		if card.category == "Weapon": color_Health = red if health < card.health_max else (white if card.health_max <= cardType.durability else green)
		...
		scale_0 = {"Hero": 1.1, "Weapon": 0.9, "Minion": 0.8}[card.category] if self.isPlayed else statTextScale
		...
		s = '' if card.category == "Hero" and attack < 1 else str(attack)
		...
		if card.category == "Hero": para.append(Func(self.models["attack_Played"].show if attack > 0 else self.models["attack_Played"].hide))
		...
		if action in ("set", "buffDebuff"):  #一般的攻击力和耐久度重置计算也可以用set，只是需要把num1和num2设为0
			...
			if card.category == "Hero":
				...
				para.append(Func(self.models["armor_Played"].hide if armor < 1 else self.models["armor_Played"].show))
			...
			if card.category == "Weapon": s, color, texCardName, num = '', white, '', 48
			...
			if texCardName:
				texCard, seqNode = makeTexCard(self.GUI, "TexCards\\Shared\\%s.egg" % texCardName, scale=3 if card.category == "Minion" else 4.5)
				textNodePath = None
				if s: (textNodePath := makeText(np_Parent=texCard, textName='', valueText=s, pos=(0, -0.01, -0.05), scale=0.25, color=color,
												font=self.GUI.font)[0]).setHpr(0, 0, 0)
				para.append(Func(self.addaStatusTexCard2Top, texCard, seqNode, 1, num, True, 0.15, 0.45 if card.category == "Minion" else 0.25, textNodePath))

	def placeIcons(self):
		card, category = self.card, self.card.category
		if not self.isPlayed and category in ("Hero", "Spell"): return
		card, para, icons = self.card, Parallel(name="Icon Placing Ani"), []
		#Minion/Weapon/Power/Dormant
		...
		if self.isPlayed and any(trig.show and trig.counter < 0 and not trig.oneTime for trig in card.trigsBoard):
		...
		if "SpecTrig" in self.icons:
			if self.isPlayed and category in ("Minion", "Weapon") and any(trig.oneTime for trig in card.trigsBoard):
		...
		nameTagModel = self.models["nameTag"] if category in ("Minion", "Dormant", "Weapon") else None
		if icons:
			if nameTagModel:
				darkNameTagImg = "Models\\%sModels\\DarkNameTag.png"%("Weapon" if category == "Weapon" else "Minion")
...
tableTexCards_Minions = {"Aura": (5.8, (0, 0.3, -0.03)),
						...
						"Cost Health Instead": (2.6, (-1.77, 1.75, 0.09)),
...
class Btn_Minion(Btn_Card):
	...
	def effectChangeAni(self, name=''):
		...
		if not name or name == "Aura":  #Check the Aura animation
			para.append(Func(self.texCardAni_LoopPose, "Aura", self.isPlayed and card.onBoard and card.auras))
		if not name or name == "Immune":
			para.append(Func(self.texCardAni_LoopPose, "Immune", self.isPlayed and card.onBoard and card.effects["Immune"] > 0))
		if not name or name == "Spell Damage":
			para.append(Func(self.texCardAni_LoopPose, "Spell Damage", self.isPlayed and card.onBoard and card.effects["Spell Damage"] + card.effects["Nature Spell Damage"] > 0))
		if not name or name == "Stealth":
			para.append(Func(self.texCardAni_LoopPose, "Stealth", self.isPlayed and card.onBoard and card.effects["Stealth"] + card.effects["Temp Stealth"] > 0))
		if not name or name in ("Exhausted", "Rush", "Charge"):
			para.append(Func(self.texCardAni_LoopPose, "Exhausted", self.isPlayed and card.onBoard and card.enterBoardTurn == card.Game.numTurn and not card.actionable()))
		if not name or name == "Silenced":
			para.append(Func(self.texCardAni_LoopPose, "Silenced", self.isPlayed and card.onBoard and card.silenced))
			para.append(Func(self.texCardAni_OnOff, "SilencedText", not self.isPlayed and card.silenced))

		#Handle the play(num1, num2)/play(num1, num2) category of texCards: Divine Shield, Taunt
		if not name or name == "Divine Shield":
			if card.onBoard: para.append(Func(self.texCardAni_PlayPlay, "Divine Shield", 23, 24, 29, self.isPlayed and card.effects["Divine Shield"] > 0))
			else: para.append(Func(self.texCardAni_LoopPose, "Divine Shield", False))
		if not name or name == "Taunt":
			if card.onBoard: para.append(Func(self.texCardAni_PlayPlay, "Taunt", 13, 14, 28, self.isPlayed and card.effects["Taunt"] > 0))
		...
		if not name or name == "Frozen":
			if card.onBoard: para.append(Func(self.texCardAni_LoopPlay, "Frozen", 96, 97, 105, self.isPlayed and card.effects["Frozen"] > 0))
...
tableTexCards_Spells = {"Cost Health Instead": (2.6, (-1.77, 1.75, 0.09)),
						"Unplayable": (3.7, (0, -0.5, 0.09)),
						}
...
def loadSpell(base):
	...
	for name, posScale in tableTexCards_Spells.items():
		makeTexCard(base, "TexCards\\ForSpells\\%s.egg" % name,
					pos=posScale[1], scale=posScale[0], name=name)[0].reparentTo(root)
...
class Btn_Weapon(Btn_Card):
	...
	def decideColor(self):
		...
		color, card = transparent, self.card
		cardType, GUI, game = card.category, self.GUI, card.Game
...
def loadWeapon(base):
	...
	makeText(root, "health_Played", '0', pos=(1.27, -0.8, 0.132), scale=0.9,
				  color=white, font=font)
	makeText(root, "description", '0', pos=(0.7, -2, 0.1), scale=0.5,
				  color=black, font=font, cardColor=yellow)
...
class Btn_Power(Btn_Card):
	...
	def effectChangeAni(self, name=''):
		...
		if not name or name in ("Damage Boost", "Power Damage"):
			para.append(Func(self.texCardAni_LoopPose, "Power Damage",
							 self.isPlayed and card.onBoard and card.canDealDmg() and card.effects["Damage Boost"] + card.Game.effects[card.ID]["Power Damage"] > 0))
		if not name or name in ("Can Target Minions", "Power Can Target Minions"):
			para.append(Func(self.texCardAni_LoopPose, "Can Target Minions",
							 self.isPlayed and card.onBoard and card.effects["Can Target Minions"] + card.Game.effects[card.ID]["Power Can Target Minions"] > 0))
...
class Btn_Hero(Btn_Card):
	...
	def effectChangeAni(self, name=''):
		...
		if not name or name == "Immune":
			para.append(Func(self.texCardAni_LoopPose, "Immune", self.isPlayed and card.onBoard and card.Game.effects[card.ID]["Immune"] > 0))
		if not name or name == "Spell Damage":
			para.append(Func(self.texCardAni_LoopPose, "Spell Damage", self.isPlayed and card.onBoard and card.Game.effects[card.ID]["Spell Damage"] > 0))
		if not name or name == "Temp Stealth":
			para.append(Func(self.texCardAni_LoopPose, "Stealth", self.isPlayed and card.onBoard and card.effects["Temp Stealth"] > 0))
		##Handle the loop(False, num1, num2)/play(num1, num2) category of texCards: Frozen
		if not name or name == "Frozen":
			para.append(Func(self.texCardAni_LoopPlay, "Frozen", 96, 97, 105, self.isPlayed and card.onBoard and card.effects["Frozen"] > 0))
...
def genOption(GUI, option, pos=None, onlyShowCardBack=False):
	...
	if onlyShowCardBack:
		...
		if not option.isLegendary: nodePath.find("legendaryIcon").hide()
...
def genCard(GUI, card, isPlayed, pickable=True, pos=None, hpr=None, scale=None,
	...
	typeofCard = card.category if card.category != "Dormant" else "Minion"
