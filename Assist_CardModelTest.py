...
from Parts_Game import Game
from HS_AcrossPacks import *
from HS_Core import *#MindSpike, LordJaraxxus, StonetuskBoar, DeathwingtheDestroyer, GrommashHellscream
...
class VariousTest(Panda_UICommon):
	...
	def testEntireGameDisplay(self):
		...
		for ID in (1, 2):
			...
			self.seqHolder[-1].append(self.heroZones[ID].seq_PlaceTurnTrigs())
		...
		#self.heroExplodeAni([self.Game.heroes[ID] for ID in (1, 2)])
		...
		self.Game.Hand_Deck.hands[1][3].getsEffect("Cost Health Instead")
		self.Game.Hand_Deck.hands[1][-1].getsEffect("Unplayable")
		print("Card to gain is", self.Game.Hand_Deck.hands[1][-1].name)
		self.seqHolder.pop(0).start()
...
class SmallTest(ShowBase):
	def __init__(self):
		super().__init__()
		model = self.loader.loadModel("Models\\Trade.glb")
		print(model.name)
		#textNode = TextNode('1')
		...
		#trig = self.loader.loadModel("Models\\HeroModels\\TurnTrig.glb")
		#trig.reparentTo(self.render)
		#icon = trig.find("Icon")
		#trig.setPosHpr(0, 0, 0, 0, 90, 0)
		#tex1 = self.loader.loadTexture("Images\\HeroesandPowers\\Fireblast.png")
		#tex2 = self.loader.loadTexture("Images\\Dragons\\ArcaneBreath.png")
		#print(icon.findTextureStage('0'), icon.findTextureStage('1'))
#
		#icon.setTexture(icon.findTextureStage('0'), tex2, 1)
		#ts = TextureStage('ForPower')
		#ts.setTexcoordName('1')
		#icon.setTexture(ts, tex1, 1)
