#from Parts_ConstsFuncsImports import findFilepath
...
def makeCardPool(monk=0, SV=0, writetoFile=True):
	...
	cardPool += AcrossPacks_Cards_Collectible #Has the basic hero and hero power definitions.
	cardPool_All += AcrossPacks_Cards #Has the basic hero and hero power definitions.
	info += "from HS_AcrossPacks import *\n"

	cardPool += Core_Cards_Collectible
	cardPool_All += Core_Cards
	info += "from HS_Core import *\n"

	cardPool += Outlands_Cards_Collectible
	cardPool_All += Outlands_Cards
	info += "from HS_Outlands import *\n"

	cardPool += Academy_Cards_Collectible
	cardPool_All += Academy_Cards
	info += "from HS_Academy import *\n"

	cardPool += Darkmoon_Cards_Collectible
	cardPool_All += Darkmoon_Cards
	info += "from HS_Darkmoon import *\n"

	cardPool += Barrens_Cards_Collectible
	cardPool_All += Barrens_Cards
	info += "from HS_Barrens import *\n"

	cardPool += Stormwind_Cards_Collectible
	cardPool_All += Stormwind_Cards
	info += "from HS_Stormwind import *\n"
	#if monk:
	...
	for card in cardPool[:]:
		if card.category == "Hero" and "~" not in card.index:
	...
	if writetoFile:
		with open("DB_CardPools.py", "w") as out_file:
			out_file.write(info)
			...
			# 把cardPool_All写入python里面
			out_file.write("cardPool_All = [")
			i = 1
			for card in cardPool_All:
				out_file.write('%s, ' % card.__name__)
				if i % 10 == 0: out_file.write('\n\t\t\t')
				i += 1
			out_file.write("\t\t]\n\n")
			...
			for poolIdentifier, objs in RNGPools.items():
				if isinstance(objs, list):
					...
					for obj in objs:
						...
						if i % 10 == 0: out_file.write('\n\t\t\t\t\t\t')
					out_file.write("],\n")
				#elif type(obj) == category({}): #专门给了不起的杰弗里斯提供的
				#	out_file.write("\t\t\t'%s': {\n"%poolIdentifier)
	...
	return cardPool, cardPool_All, RNGPools
...
class Label_CardinDeck(tk.Label):
	def __init__(self, master, UI, card, count=1):
		...
		super().__init__(master=master, text=self.decideText(), #"%d  " % card.mana + card.name_CN,
						 font=("Yahei", 15, "bold") if card.category == "Minion" else ("Yahei", 15),
...
class DeckBuilderWindow(tk.Tk):
	...
	def updateDeckLabels(self):
		...
		indices_ArgSort = numpy.array([card.mana for card in cards]).argsort()
...
if __name__ == "__main__":
	...
	from DB_CardPools import ClassCards, NeutralCards
