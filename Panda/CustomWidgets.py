...
OpeningAngle = 40
radius = 12.9
HandZone_Z, HandZone1_Y, HandZone2_Y = 2.5, -25, 26.2
...
class HandZone:
	...
	def placeCards(self, add2Queue=True): #Sometimes, CardModelTest.py wants to use this without the seqHolder
		...
		for i, card in enumerate(ownHand):
			genCard(GUI, card, isPlayed=False, scale=scale_Hand, onlyShowCardBack=GUI.need2Hide(card), seq=para)
		for i, card in enumerate(ownHand):
			if card.btn != GUI.btnBeingDragged and card not in GUI.playedCards:
...
class HeroZone:
	def __init__(self, GUI, ID):
		...
		self.manaText.setFont(self.GUI.getFont())
	...
	def heroReplacedAni(self, oldHero, oldPower):
		print("Hero replace ani", oldHero, oldPower)
		...
		self.placeCards()
		print("Hero replace Ani before adding remove nodepath")
	...
	def placeCards(self, add2Queue=True, powerFlip=True, weaponDescends=True):
		...
		para.append(Sequence(Func(print, "Reparenting hero", hero, hero.btn.np),
							 Func(hero.btn.np.reparentTo, GUI.render)))
		...
		if powerFlip:
			seq = Sequence(Func(print, "Reparenting weapon", power, power.btn.np),
							Func(power.btn.np.reparentTo, GUI.render),
	...
	def drawMana(self, usable, upper, locked, overloaded):
		...
		#古夫在设置水晶上限为20之后，除了数值显示外，其他的仍为最多为10个
		manas, upper = self.GUI.manaModels, min(10, upper)
		usable, locked, overloaded = min(usable, upper), min(10, locked), min(10, overloaded)
		if self.heroPos[1] < 0:
...
class Btn_Board:
	...
	def leftClick(self):
		...
		if -1 < self.GUI.stage < 3:  #不在发现中和动画演示中才会响应
...
class DeckZone:
	def __init__(self, GUI, ID):
		...
		self.textNodePath, self.textNode = makeText(GUI.render, "Text2Keep", "Deck: {}\nHand: {}".format(0, 0),
													pos=(self.pos[0]+4, self.pos[1]+0.3, self.pos[2]-1), scale=1,
													color=white, font=GUI.getFont(GUI.lang))
		self.textNodePath.wrtReparentTo(np_Deck)

		(cNode := CollisionNode("cNode")).addSolid(CollisionBox((0.2, 0, -1.1), 0.8, 2, 3))
		self.np_Deck.attachNewNode(cNode)#.show()
	...
	def fatigueAni(self, numFatigue):
		textNode = self.GUI.np_Fatigue.find("Fatigue_TextNode").node()
		...
		if self.GUI.seqHolder: self.GUI.seqHolder[-1].append(Sequence(Func(textNode.setText, str(numFatigue)), interval,
																	  Wait(1), Func(self.GUI.np_Fatigue.setPos, 0, 0, 0 ) ))
		else: Sequence(Func(textNode.setText, str(numFatigue)), interval,
					   Wait(1), Func(self.GUI.np_Fatigue.setPos, 4, 0, 0) ).start()
		
	def leftClick(self):
		if self.GUI.stage == 2: self.GUI.resolveMove(self.GUI.Game.Hand_Deck, None, "Deck")
...
class GameEvent:
	def __init__(self, GUI, subject, eventType, target, textSubject, textTarget, colorSubject, colorTarget, level):
		...
		if target:
			if isinstance(target, (tuple, list)): self.targets = list(target)
		...
		if textTarget:
			if isinstance(textTarget, (tuple, list)): textTarget = list(textTarget)
	...
	def make_npSubject(self, subject, eventType, textSubject, colorSubject):
		if subject:
			...
			if subject.race == "Secret" and self.GUI.need2Hide(subject):
	...
	def make_npTarget(self, obj, parent, text='', color=white):
		...
		if obj.race == "Secret" and self.GUI.need2Hide(obj):
...
class HistoryZone:
	...
	def placeIcons(self, events2Remove=()):  # 创建event和向event中添加target都会重新计算event.numRows
		...
		for event in self.events:
			indexRow += max(1, math.ceil(len(event.targets) / iconsaRow))
	...
	def add2Event(self, subject, target, textSubject='', textTarget='', colorSubject=white, colorTarget=white):
		...
		if event:
			...
			if target:
				...
				if not isinstance(textTarget, (list, tuple)): textTarget = [textTarget]
